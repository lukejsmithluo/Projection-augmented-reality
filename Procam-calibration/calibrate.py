...
def _detect_chessboard(white_img: np.ndarray, chess_shape: tuple[int, int]):
    standard_flags = (
    ...
    variants: list[np.ndarray] = [
        white_img,
        clahe(white_img),
        cv2.GaussianBlur(white_img, (5, 5), 0),
        cv2.GaussianBlur(clahe(white_img), (5, 5), 0),
        gamma(white_img, 0.7),
        gamma(clahe(white_img), 0.7),
    ]

    for img in variants:
        res, corners = cv2.findChessboardCorners(img, chess_shape, standard_flags)
        if res:
            return True, corners, img

    find_sb = getattr(cv2, "findChessboardCornersSB", None)
    if find_sb is not None:
        sb_flags = cv2.CALIB_CB_EXHAUSTIVE | cv2.CALIB_CB_ACCURACY
        for img in variants:
            res, corners = find_sb(img, chess_shape, sb_flags)
            if res:
                return True, corners, img
...
def main():
    ...
    parser.add_argument(
        "graycode_step", type=int, default=1, help="step size of graycode"
    )
    parser.add_argument(
        "-black_thr",
        type=int,
    ...
    parser.add_argument(
        "-camera", type=str, default=str(), help="camera internal parameter json file"
    )
    parser.add_argument(
        "-input_dir",
        type=str,
        default="./",
        help="directory containing capture_* folders (default: ./)",
    )
    parser.add_argument(
        "-output_xml",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=str(),
        help="tag prefix for visualization images (default: empty)",
    )
    parser.add_argument(
        "-enable_subpix",
        type=int,
        default=1,
        help="enable corner subpixel refinement (default: 1)",
    )
    parser.add_argument(
        "-enable_view_filter",
        type=int,
        default=1,
        help="enable outlier view filtering (default: 1)",
    )
...
def calibrate(
    ...
    for dname, gc_filenames in zip(dirnames, gc_fname_lists):
        ...
        res, cam_corners, chess_img = _detect_chessboard(white_img, chess_shape)
        if not res:
            print(
                "  Warning : chessboard was not found in '"
                + gc_filenames[-2]
        ...
        if enable_subpix:
            ...
            cv2.cornerSubPix(chess_img, cam_corners, (11, 11), (-1, -1), term_crit)
        cols = chess_shape[0]
        rows = chess_shape[1]
        ...
        if cols > 1:
            dists.extend(
                np.linalg.norm(grid[:, 1:, :] - grid[:, :-1, :], axis=2)
                .reshape(-1)
                .tolist()
            )
        if rows > 1:
        ...
        if len(proj_corners) < min_view_corners:
            print(
                "  Warning : too few corners were found in '"
                + dname
                + "' ("
                + str(len(proj_corners))
                + " < "
                + str(min_view_corners)
                + "), this view will be skipped"
            )
    ...
    for i, (objp, corners, rvec, tvec) in enumerate(
        zip(proj_objps_list, proj_corners_list, proj_rvecs, proj_tvecs)
    ):
    ...
    if enable_view_filter and len(proj_view_errors) > 0:
        ...
        if 0 < len(kept_indices) < len(proj_objps_list):
            removed = [
                (
                    view_names[i] if i < len(view_names) else str(i),
                    float(proj_view_errors[i]),
                )
                for i in range(len(proj_view_errors))
            ...
            print(
                "    Kept : "
                + str(len(kept_indices))
                + " / "
                + str(len(proj_view_errors))
            )
            ...
            for i, (objp, corners, rvec, tvec) in enumerate(
                ...
                imgpts, _ = cv2.projectPoints(objp, rvec, tvec, proj_int, proj_dist)
                error = np.sqrt(
                    cv2.norm(corners, imgpts, cv2.NORM_L2) ** 2 / len(imgpts)
                )
                view_name = view_names[i] if i < len(view_names) else str(i)
                print(f"    View {i} ({view_name}): {error:.4f}")
    ...
    fs.write(
        "proj_view_rms_before",
        np.array(proj_view_errors_before, dtype=np.float64).reshape(-1, 1),
    )
