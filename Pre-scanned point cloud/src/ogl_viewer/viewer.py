...
class SubMapObj:
    ...
    def update_mesh(self, _chunk): 
        ...
        if len(_chunk.vertices):
            ...
            # Use numpy array directly for better memory safety
            glBufferData(GL_ARRAY_BUFFER, self.vert.nbytes, self.vert, GL_DYNAMIC_DRAW)
        
        if len(_chunk.triangles):
            self.tri = _chunk.triangles.flatten()      # transform _chunk.triangles into 1D array 
            # Ensure proper data type for triangle indices
            self.tri = self.tri.astype(np.uint32)
            glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, self.vboID[1])
            # Use numpy array directly instead of ctypes conversion
            glBufferData(GL_ELEMENT_ARRAY_BUFFER, self.tri.nbytes, self.tri, GL_DYNAMIC_DRAW)
    ...
    def update_fpc(self, _chunk): 
        ...
        if len(_chunk.vertices):
            ...
            # Clear previous indices and create new ones for current chunk
            self.index = []
            num_vertices = len(_chunk.vertices)
            for i in range(num_vertices):
                self.index.append(i)
            
            # Ensure proper data type for OpenGL buffer
            index_np = np.array(self.index, dtype=np.uint32)
            glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, self.vboID[1])
            
            # Use numpy array directly instead of ctypes conversion for better memory safety
            glBufferData(GL_ELEMENT_ARRAY_BUFFER, index_np.nbytes, index_np, GL_DYNAMIC_DRAW)
