...
class CaptureService:
    ...
    def initialize(self, proj_width: int, proj_height: int, graycode_step: int = 1, monitor_index: int = 1):
        if not sl:
    ...
    def _setup_window(self):
        ...
        if len(monitors) <= self.monitor_index:
            ...
            print(f"Warning: Monitor index {self.monitor_index} not found. Using primary monitor.")
        else:
        ...
        cv2.imshow(self.window_name, np.zeros((self.proj_height, self.proj_width), np.uint8))
        cv2.waitKey(100)
    ...
    def capture_pose(self, capture_idx: int) -> bool:
        ...
        if not self._is_initialized:
            raise RuntimeError("CaptureService not initialized. Call initialize() first.")
